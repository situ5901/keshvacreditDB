...
const PINCODE_FILE_PATH = path.join(__dirname, "..", "xlsx", "olyv.xlsx");
...
async function loadValidPincodes() {
  try {
    // FIX 2: Added backticks around the entire string
    ...
    // FIX 3: Added backticks around the entire string
  ...
  const promises = leads.map(async (lead) => {
    try {
      ...
      if (!leadPincode || !validPincodes.has(leadPincode)) {
        // FIX 4: Added backticks around the entire string
        console.error(
          ...
          { phone: lead.phone },
          {
            $push: {
              RefArr: {
                ...
                // FIX 5: Added backticks around the entire string
      ...
      if (!lead.phone || !lead.name || !lead.dob || !lead.pan) {
        // FIX 6: Added backticks around the entire string
        ...
        await UserDB.updateOne(
          { phone: lead.phone },
          {
            $push: {
              RefArr: {
                name: "SkippedSmartcoin",
                // FIX 8: Added backticks around the entire string
      ...
      // FIX 9: Added backticks around the entire string
      ...
      if (
        ...
        successCount++;
        // FIX 10: Added backticks around the entire string
        console.log(`✅ Lead counted as success: ${lead.phone}`);
      } else {
        // FIX 11: Added backticks around the entire string
...
async function Loop() {
  try {
    ...
    while (true) {
      ...
      // FIX 12: Added backticks around the entire string
      console.log(`🏁 Total Successful SmartCoin Leads: ${successCount}`);
      // FIX 13: Added backticks around the entire string
    ...
    // FIX 14: Added backticks around the entire string
    console.log(`🏁 Total Successful SmartCoin Leads: ${successCount}`);
